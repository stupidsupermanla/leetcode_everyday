...
public class P733FloodFill {
    public static void main(String[] args) {
        ...
        int[][] ints = solution.floodFill(image, 1, 1, 1);
        for (int[] anInt : ints) {
            for (int i : anInt) {
                System.out.print(i);
            }
    ...
    class Solution {
        public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
            ...
            int yMax = image.length;
            int xMax = image[0].length;
            ...
            while (!queue.isEmpty()) {
                ...
                image[y][x] = newColor;
                for (int i = 0; i < 4; i++) {
                    if (y + ys[i] >= 0 && y + ys[i] < yMax && x + xs[i] >= 0 && x + xs[i] < xMax && image[y + ys[i]][x + xs[i]] == oldColor) {
                        queue.add(new Pair<>(y + ys[i], x + xs[i]));
                    }
                }
            }
            return image;
